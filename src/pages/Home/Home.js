...
function Home() {
  ...
  async function setup() {
    try{
      const {data} = await axios.get('https://api.nytimes.com/svc/topstories/v2/home.json?api-key=G3Zxr0y95aXHyAInrBCbNtumQd5BKg1h');
  ...
  const FILTERED_BUSINESS = apiResult.filter(news => (news.section === "business"))
    ...
    <div>
      <div style={{background:'lightblue'}}>
       {FILTERED_ARTS.map( (news, i) => (
        <>
          <p style={{color:'red'}}>{news.section} : {news.subsection}</p>
          <News key={i}  
            title={news.title} 
            abstract={news.abstract}
            url={news.url} 
            image={news.multimedia}
